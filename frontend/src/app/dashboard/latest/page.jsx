...
export default function LatestDashboard() {
  ...
  async function fetchItems() {
    try {
      const res = await fetch(`${apiBase}/latest`, { cache: "no-store" });
  ...
  async function saveEdit(id) {
    ...
    try {
      ...
      if (editFields.summary !== undefined)
        fd.append("summary", editFields.summary);
      if (editFields.link !== undefined) fd.append("link", editFields.link);
      if (editFields.imageFile) fd.append("image", editFields.imageFile);

      const res = await fetch(`${apiBase}/latest/${id}`, {
        method: "PUT",
        body: fd,
      });
      if (!res.ok) {
  ...
  const inputClass =
    "p-2 bg-gray-800 text-white rounded border border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500";
  const fileInputClass =
    "p-2 bg-gray-800 text-white rounded border border-gray-700 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-gray-700 file:text-gray-300 hover:file:bg-gray-600";
  ...
  return (
    <div className="p-6 w-full min-h-screen">
      <div>
        ...
        <h2 className="text-2xl text-white mb-4">Latest In Tech</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((it) => (
          <div
            key={it._id}
            className="p-4 bg-gray-800 text-white rounded shadow-lg flex flex-col justify-between"
          >
            {editingId === it._id ? (
              // --- Edit State ---
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col">
                  <label
                    htmlFor={`edit-summary-${it._id}`}
                    className={labelClass}
                  >
                    Summary
                  </label>
                  <textarea
                    id={`edit-summary-${it._id}`}
                    value={editFields.summary}
                    onChange={(e) =>
                      setEditFields((s) => ({ ...s, summary: e.target.value }))
                    }
                    className={`${inputClass} min-h-[80px]`}
                    rows={3}
                  />
                ...
                <div className="flex flex-col">
                  <label htmlFor={`edit-link-${it._id}`} className={labelClass}>
                    Link
                  </label>
                  <input
                    id={`edit-link-${it._id}`}
                    ...
                    onChange={(e) =>
                      setEditFields((s) => ({ ...s, link: e.target.value }))
                    }
                    className={inputClass}
                  />
                </div>
              ...
              <div>
                ...
                {it.imageUrl && (
                  <img
                    src={it.imageUrl}
                    alt={it.title || "Tech Image"}
                    className="my-2 rounded-md w-full h-48 object-cover"
                  />
                )}
                ...
                {it.link && (
                  <a
                    ...
                    className="text-blue-400 hover:text-blue-300 text-sm truncate block font-medium"
                  >
                    Read more
                  </a>
                )}
